...
const GetTreatmentPage = () => {
    ...
    const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api/auth/get-treatment';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(API_URL, { symptom });
            if (response.data.success) {
                setTreatmentData(response.data.data);
                simulateTypingEffect(response.data.data);
            } else {
                setError('No treatment data available');
            }
        } catch (err) {
            const errorMessage = err.response?.data?.message || "Error fetching treatment data. Please try again.";
